...
function createMetadataJSON(server) {
    ...
    const getAllOperations = function (target) {
    ...
    const propNames = utils_1.getAllPropertyNames(server.prototype).filter(it => it != "constructor");
    ...
    const resolveTypeDefinition = (elementType, prop, namespace, getType = Edm.getType, getTypeName = Edm.getTypeName) => {
        ...
        if (!defName) {
            ...
            defDefinition = {
                name: defName,
                underlyingType: getTypeName(defType, undefined, server.container) || "System.Object"
            };
        }
        else {
            if (defName.indexOf(".") > 0) {
    ...
    const resolveEnumType = (elementType, prop, namespace, enumType, enumName) => {
        ...
        if (!enumName) {
            ...
            enumDefinition = {
                ...
                member: [],
                annotation: Edm.getAnnotations(enumType)
            };
        }
        else {
            ...
            Object.keys(enumType).forEach((member) => {
                if (!(/^[0-9@]/.test(member) || enumType[member] == "@odata.type")) {
                    enumDefinition.member.push({
                        ...
                        annotation: Edm.getAnnotations(enumType, member)
                    });
                }
            });
        ...
        let parameters = Edm.getParameters(elementType, prop).map(p => {
            ...
            if (typeof param.type != "string") {
                ...
                if (Edm.isTypeDefinition(param.type)) {
                    param.type = resolveTypeDefinition(containerType, resolvedContainerType, namespace, Edm.getType, () => {
